class Cosmoslike::TransactionSearch
  ...
  def initialize(chain, attrs = {})
    ...
    if @after_time.present? && @before_time.present? && (@after_time > @before_time)
      raise Cosmoslike::TransactionSearch::Error, 'Start Date must be less than or equal to End Date'
    end
    ...
    if @after_height && @before_height && (@after_height > @before_height)
      raise Cosmoslike::TransactionSearch::Error, 'Start Height must be less than or equal to End Height'
    end
  end
